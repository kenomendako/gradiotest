...
def format_history_for_gradio(messages: List[Dict[str, str]]) -> List[Dict[str, Union[str, tuple, None]]]:
    ...
    for i, msg in enumerate(messages):
        ...
        delete_button = f"<a href='#delete_action' title='この発言を削除' style='padding: 1px 6px; font-size: 1.0em; text-decoration: none; color: #555;'>🗑️</a>"
        button_container = f"<div style='text-align: right; margin-top: 8px;'>{up_button} {down_button} <span style='margin: 0 4px;'></span> {delete_button}</div>"
...
def save_message_to_log(log_file_path: str, header: str, text_content: str) -> None:
    ...
    try:
        ...
        if os.path.exists(log_file_path) and os.path.getsize(log_file_path) > 0:
            with open(log_file_path, "rb") as f:
                try:
                    ...
                    f.seek(0)
                    if f.read(): # Check if file is not empty
                        needs_leading_newline = True
        ...
        # ファイルが空、または存在しない場合は先頭の改行は不要
        if not (os.path.exists(log_file_path) and os.path.getsize(log_file_path) > 0):
             content_to_write = f"{header}\n{text_content.strip()}"
        else:
             content_to_write += f"{header}\n{text_content.strip()}"
...
def delete_message_from_log(log_file_path: str, message_to_delete: Dict[str, str]) -> bool:
    ...
    try:
        ...
        if not found_and_deleted:
            print(f"警告: ログファイル内に削除対象のメッセージが見つかりませんでした。Content: {content_to_find[:50]}...")
            return False
