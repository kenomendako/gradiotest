...
def format_history_for_gradio(messages: List[Dict[str, str]]) -> List[Dict[str, Union[str, tuple, None]]]:
    ...
    for i, msg in enumerate(messages):
        ...
        # 削除ボタンは、selectイベントを意図的に発火させるために、特別なhrefを持たせる
...
def save_message_to_log(log_file_path: str, header: str, text_content: str) -> None:
    if not log_file_path or not header or not text_content or not text_content.strip(): return
    try:
        ...
        if os.path.exists(log_file_path) and os.path.getsize(log_file_path) > 0:
            with open(log_file_path, "rb") as f:
                ...
                if f.read() != b'': needs_leading_newline = True
