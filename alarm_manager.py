...
def add_alarm(hour: str, minute: str, character: str, theme: str, flash_prompt: str, days_ja: list):
    global alarms_data_global
    if not character:
        # UI specific error removed
        print("エラー: キャラクターが選択されていません。")
...
def delete_alarm(alarm_id: str): # Renamed from delete_alarm_interactive
    global alarms_data_global
...
def get_all_alarms():
    """Returns the current list of all alarms."""
    load_alarms() # Ensure data is fresh
    return alarms_data_global
...
def check_alarms():
    ...
    # Load alarms directly within the check, ensuring it's fresh
    # No longer relying on alarms_data_global directly from module scope for this check loop
    current_alarms = load_alarms()

    if not current_api_key:
        return

    for a in current_alarms: # Iterate over the freshly loaded alarms
        alarm_time = a.get("time")
        is_enabled = a.get("enabled")
        alarm_id_for_log = a.get('id', 'N/A')
        alarm_days = a.get("days", [])

        if alarm_days is None: alarm_days = []
        if not alarm_days: continue
...
def start_alarm_scheduler_thread():
    ...
    # Ensure config is loaded before starting thread, especially if it relies on global config vars
    config_manager.load_config() # Make sure globals like API keys, model are loaded.

    # Start the scheduler in a new thread if not already running or if stopped
    # Basic check to prevent multiple threads; more robust checking might be needed in a complex app
...
def stop_alarm_scheduler_thread():
    ...
    if hasattr(start_alarm_scheduler_thread, "scheduler_thread") and \
       start_alarm_scheduler_thread.scheduler_thread.is_alive():
        ...
        start_alarm_scheduler_thread.scheduler_thread.join(timeout=5) # Wait for thread to stop
...
if __name__ == '__main__':
    print("alarm_manager.py - 直接実行テストモード")

    # 設定ファイルのロード（テスト用に必要に応じてパス調整）
    # config_manager.CONFIG_FILE = 'config.json' # Adjust if your test setup needs it
    # config_manager.ALARMS_FILE = 'alarms.json' # Adjust
    config_manager.load_config()
    load_alarms()

    print(f"読み込まれたアラーム ({len(alarms_data_global)}件):")
    for alm in alarms_data_global:
        print(f"  ID: {alm['id']}, Time: {alm['time']}, Char: {alm['character']}, Enabled: {alm['enabled']}, Days: {alm['days']}")

    # テスト用アラーム追加
    # print("\nテストアラーム追加:")
    # add_alarm("00", "00", "test_char", "テストテーマ", None, ["月", "火"]) # Hour, Minute, Char, Theme, FlashP, DaysJA
    ...
    # スケジューラスレッドのテスト（短時間）
    # print("\nスケジューラスレッドテスト開始（10秒間）...")
    # start_alarm_scheduler_thread()
    # time.sleep(10)
    # stop_alarm_scheduler_thread()
    # print("スケジューラスレッドテスト終了。")
