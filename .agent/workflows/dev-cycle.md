...
### 3.1 ブランチ作成と確認
⚠️ **mainブランチへの直接コミットは厳禁です**
必ず作業用ブランチを作成し、切り替わったことを確認してから実装を開始すること。
...
### 4.4 承認要求（🛑 STOP）
**これ以降の操作は、ユーザーの明示的な承認および動作確認が必要です。**
`notify_user` ツールを呼び出し、以下の内容を報告して指示を待ってください。
1. **変更内容の概要**
2. **動作確認の結果**
3. **「マージしてメインブランチに反映してもよろしいですか？」と尋ねる**

### 4.5 マージ & プッシュ
**ユーザーからの承認が得られた場合のみ実行する。**
決して独断でマージしてはならない。
```bash
git checkout main
