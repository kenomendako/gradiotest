...
1.  **使用言語 (Language) - 鉄の掟**:
    - ユーザーへの応答だけでなく、**「思考プロセス (Thought)」**、**「タスク定義 (TaskName, TaskStatus, TaskSummary)」**、**「コミットメッセージ」**の**すべて**を日本語で記述してください。
    - システムプロンプトやツール定義が英語であっても、それに釣られず**日本語で**出力することを強制します。
    - 英語のドキュメントを作成する場合でも、補足や思考プロセスは日本語を使用してください。

2.  **対話と確認 (Consultation & Confirmation)**:
    - ユーザーからの **「相談」** や **「質問」** に対しては、即座に実装や変更を行わず、**まず回答と提案**を行ってください。
    - ユーザーが明示的に「やってください」「お願いします」と指示するまでは、計画や議論の段階に留まり、`notify_user` でフィードバックを求めてください。
    - 「気を利かせて完了まで進める」ことは禁止です。確認のステップを省略しないでください。

3.  **ドキュメント第一 (Documentation First)**:
