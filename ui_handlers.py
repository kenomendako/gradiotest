...
# (handle_message_submission や他の既存の関数 ... 変更なし)
def _generate_initial_scenery(character_name: str, api_key_name: str) -> Tuple[str, str]:
    ...
    location_display_name = location_id; space_def = "（現在の場所の定義・設定は、取得できませんでした）"; scenery_text = "（場所の定義がないため、情景を描写できません）"
    try:
        if not space_details_raw.startswith("【エラー】"):
            try: space_data = json.loads(space_details_raw)
            if isinstance(space_data, dict): location_display_name = space_data.get("name", location_id); space_def = json.dumps(space_data, ensure_ascii=False, indent=2)
        ...
        if not space_def.startswith("（"):
            ...
            scenery_text = llm_flash.invoke(scenery_prompt).content; print(f"  - 生成された情景: {scenery_text}")
    except Exception as e: print(f"--- [軽量版] 情景生成中にエラー: {e}"); traceback.print_exc(); location_display_name = "（エラー）"; scenery_text = "（情景生成エラー）"
...
def handle_message_submission(*args: Any):
    ...
    if not user_prompt_from_textbox and not file_input_list:
        ...
        yield chatbot_history, gr.update(), gr.update(), token_count, gr.update(), gr.update(); return
    log_message_parts = []
    if user_prompt_from_textbox: timestamp = f"\n\n{datetime.datetime.now().strftime('%Y-%m-%d (%a) %H:%M:%S')}" if add_timestamp_checkbox else ""; processed_user_message = user_prompt_from_textbox + timestamp; chatbot_history.append({"role": "user", "content": processed_user_message}); log_message_parts.append(processed_user_message)
    if file_input_list:
        for file_obj in file_input_list: filepath = file_obj.name; filename = os.path.basename(filepath); safe_filepath = os.path.abspath(filepath).replace("\\", "/"); md_string = f"[{filename}](/file={safe_filepath})"; chatbot_history.append({"role": "user", "content": md_string}); log_message_parts.append(f"[ファイル添付: {filepath}]")
    ...
    except Exception as e: traceback.print_exc(); response_data = {"response": f"[UIハンドラエラー: {e}]", "location_name": "（エラー）", "scenery": "（エラー）"}
    ...
    log_f, _, _, _, _ = get_character_files_paths(current_character_name); final_log_message = "\n\n".join(log_message_parts).strip()
    if final_log_message: user_header = utils._get_user_header_from_log(log_f, current_character_name); utils.save_message_to_log(log_f, user_header, final_log_message)
    if final_response_text: utils.save_message_to_log(log_f, f"## {current_character_name}:", final_response_text)
    raw_history = utils.load_chat_log(log_f, current_character_name); display_turns = _get_display_history_count(api_history_limit_state); formatted_history = utils.format_history_for_gradio(raw_history[-(display_turns*2):])
    token_count = update_token_count(current_character_name, current_model_name, None, None, api_history_limit_state, current_api_key_name_state, send_notepad_state, use_common_prompt_state, add_timestamp_checkbox, send_thoughts_state, send_core_memory_state, send_scenery_state)
...
def handle_scenery_refresh(character_name: str, api_key_name: str) -> Tuple[str, str]:
    ...
    gr.Info(f"「{character_name}」の現在の情景を更新しています..."); loc, scen = _generate_initial_scenery(character_name, api_key_name); gr.Info("情景を更新しました."); return loc, scen
def handle_location_change_and_update_scenery(character_name: str, location_id: str, api_key_name: str) -> Tuple[str, str]:
    ...
    if not character_name or not location_id: gr.Warning("キャラクターと移動先の場所を選択してください。"); return _generate_initial_scenery(character_name, api_key_name)
    result = set_current_location.func(location=location_id, character_name=character_name)
    if "Success" not in result: gr.Error(f"場所の変更に失敗しました: {result}"); return _generate_initial_scenery(character_name, api_key_name)
    gr.Info(f"場所を「{location_id}」に変更しました。続けて情景を更新します。"); loc, scen = _generate_initial_scenery(character_name, api_key_name); gr.Info("場所情報を更新しました."); return loc, scen
def get_location_list_for_ui(character_name: str) -> list:
    if not character_name: return []
    _, _, _, memory_json_path, _ = get_character_files_paths(character_name); memory_data = load_memory_data_safe(memory_json_path)
...
def handle_add_new_character(character_name: str):
    if not character_name or not character_name.strip(): gr.Warning("キャラクター名が入力されていません。"); char_list = character_manager.get_character_list(); return gr.update(choices=char_list), gr.update(choices=char_list), gr.update(choices=char_list), gr.update(value="")
    safe_name = re.sub(r'[\\/*?:"<>|]', "", character_name).strip()
    if not safe_name: gr.Warning("無効なキャラクター名です。"); char_list = character_manager.get_character_list(); return gr.update(choices=char_list), gr.update(choices=char_list), gr.update(choices=char_list), gr.update(value="")
    if character_manager.ensure_character_files(safe_name): gr.Info(f"新しいキャラクター「{safe_name}」さんを迎えました！"); new_char_list = character_manager.get_character_list(); return gr.update(choices=new_char_list, value=safe_name), gr.update(choices=new_char_list, value=safe_name), gr.update(choices=new_char_list, value=safe_name), gr.update(value="")
    else: gr.Error(f"キャラクター「{safe_name}」の準備に失敗しました。"); char_list = character_manager.get_character_list(); return gr.update(choices=char_list), gr.update(choices=char_list), gr.update(choices=char_list), gr.update(value=character_name)
...
def update_ui_on_character_change(character_name: Optional[str], api_history_limit_value: str):
    if not character_name: all_chars = character_manager.get_character_list(); character_name = all_chars[0] if all_chars else "Default"
    config_manager.save_config("last_character", character_name); log_f, _, img_p, mem_p, notepad_p = get_character_files_paths(character_name)
    display_turns = _get_display_history_count(api_history_limit_value)
    chat_history = utils.format_history_for_gradio(utils.load_chat_log(log_f, character_name)[-(display_turns * 2):])
    memory_str = json.dumps(load_memory_data_safe(mem_p), indent=2, ensure_ascii=False); profile_image = img_p if img_p and os.path.exists(img_p) else None
    notepad_content = load_notepad_content(character_name); locations = get_location_list_for_ui(character_name); current_location_id = utils.get_current_location(character_name)
...
def handle_initial_load():
    ...
    char_name = config_manager.initial_character_global; model_name = config_manager.initial_model_global; api_key_name = config_manager.initial_api_key_name_global; api_history_limit = config_manager.initial_api_history_limit_option_global
...
def handle_reload_memory(character_name: str) -> str:
    ...
    gr.Info(f"「{character_name}」の記憶を再読み込みしました。"); _, _, _, memory_json_path, _ = get_character_files_paths(character_name)
    memory_data = load_memory_data_safe(memory_json_path); return json.dumps(memory_data, indent=2, ensure_ascii=False)
...
def handle_clear_notepad_click(character_name: str) -> str:
    ...
    try: with open(notepad_path, "w", encoding="utf-8") as f: f.write(""); gr.Info(f"「{character_name}」のメモ帳を空にしました。"); return ""
...
DAY_MAP_EN_TO_JA = {"mon": "月", "tue": "火", "wed": "水", "thu": "木", "fri": "金", "sat": "土", "sun": "日"}; DAY_MAP_JA_TO_EN = {v: k for k, v in DAY_MAP_EN_TO_JA.items()}
def render_alarms_as_dataframe():
    alarms = sorted(alarm_manager.load_alarms(), key=lambda x: x.get("time", "")); all_rows = []
    for a in alarms:
        theme_content = a.get("context_memo") or ""; date_str = a.get("date"); days_list = a.get("days", []); schedule_display = "単発"
        if date_str:
            try: date_obj = datetime.datetime.strptime(date_str, "%Y-%m-%d").date(); today = datetime.date.today()
...
def handle_alarm_selection(evt: gr.SelectData, df_with_id: pd.DataFrame) -> List[str]:
    ...
    try: indices = [idx[0] for idx in evt.index] if isinstance(evt.index, list) else [evt.index[0]]; return [str(df_with_id.iloc[i]['ID']) for i in indices if 0 <= i < len(df_with_id)]
    except: return []
def handle_alarm_selection_and_feedback(evt: gr.SelectData, df_with_id: pd.DataFrame): selected_ids = handle_alarm_selection(evt, df_with_id); return selected_ids, "アラームを選択してください" if not selected_ids else f"{len(selected_ids)} 件のアラームを選択中"
...
def handle_timer_submission(timer_type, duration, work, brk, cycles, char, work_theme, brk_theme, api_key_name, normal_theme):
    ...
    try: timer = UnifiedTimer(timer_type, float(duration or 0), float(work or 0), float(brk or 0), int(cycles or 0), char, work_theme, brk_theme, api_key_name, normal_theme=normal_theme); timer.start(); gr.Info(f"{timer_type}を開始しました."); return f"{timer_type}を開始しました。"
...
def handle_rag_update_button_click(character_name: str, api_key_name: str):
    ...
    gr.Info(f"「{character_name}」のRAG索引の更新を開始します..."); import rag_manager; threading.Thread(target=lambda: rag_manager.create_or_update_index(character_name, api_key)).start()
def _run_core_memory_update(character_name: str, api_key: str):
    print(f"--- [スレッド開始] コアメモリ更新処理を開始します (Character: {character_name}) ---")
    try: result = memory_tools.summarize_and_save_core_memory.func(character_name=character_name, api_key=api_key); print(f"--- [スレッド終了] コアメモリ更新処理完了 --- 結果: {result}")
...
def reload_chat_log(character_name: Optional[str], api_history_limit_value: str):
    ...
    display_turns = _get_display_history_count(api_history_limit_value)
    history = utils.format_history_for_gradio(utils.load_chat_log(log_f, character_name)[-(display_turns*2):]); return history
def update_token_count(*args):
    ...
    try:
        ...
        api_key = config_manager.API_KEYS.get(current_api_key_name_state); limit_info = gemini_api.get_model_token_limits(current_model_name, api_key)
...
def handle_prime_for_deletion(chatbot_history: List[Dict[str, str]], evt: gr.SelectData):
    if not evt.value: return -1, gr.update(visible=False), ""
    try:
        # href='#delete_action' を持つリンクがクリックされた場合のみ続行
        if '#delete_action' not in evt.value: return gr.update(), gr.update(visible=False), gr.update()
        ...
        print(f"削除準備中にエラー: {e}"); return -1, gr.update(visible=False), ""

def handle_confirm_delete(primed_index: int, character_name: str, api_history_limit: str):
    if primed_index < 0: return gr.update(), gr.update(visible=False), -1
    log_f, _, _, _, _ = get_character_files_paths(character_name)
