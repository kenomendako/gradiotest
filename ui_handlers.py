...
from utils import load_chat_log, format_history_for_gradio as original_format_history_for_gradio, save_message_to_log, _get_user_header_from_log # Renamed for custom wrapper
import re # Added for file content replacement
        ...
        for message_content in pair:
            if isinstance(message_content, str):
                ...
                def replace_attached_image(match):
                    ...
                    # original_filename からMarkdown特殊文字を除去 (altテキスト用)
                    alt_text = re.sub(r"[\[\]()]", "", original_filename)
                    # file_path をURLエンコード (スラッシュはエンコードしない)
                    encoded_file_path = urllib.parse.quote(file_path, safe=' /')
                    url_path = f"/file={encoded_file_path}"
                    return f"![{alt_text}]({url_path})"
                modified_content = re.sub(ATTACHED_IMAGE_PATTERN, replace_attached_image, modified_content)

                # 3. 生成画像の置換
                #   入力形式: [Generated Image: chat_attachments/generated_images/char_prompt_uuid.jpg]
                #   出力形式: ![generated_image](/file=chat_attachments/generated_images/char_prompt_uuid.jpg)
                def replace_generated_image(match):
                    image_path = match.group(1)
                    # image_path をURLエンコード (スラッシュはエンコードしない)
                    encoded_image_path = urllib.parse.quote(image_path, safe=' /')
                    url_path = f"/file={encoded_image_path}"
                    return f"![generated_image]({url_path})"
                modified_content = re.sub(GENERATED_IMAGE_PATTERN, replace_generated_image, modified_content)

                processed_pair.append(modified_content)
            else:
                # 文字列でない場合（通常はNone）、そのまま追加
                processed_pair.append(message_content)
        processed_history.append(processed_pair)

    print(f"Debug: format_history_for_gradio_wrapper processed_history: {processed_history}") # デバッグログ追加
