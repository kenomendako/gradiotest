...
def render_alarms_as_dataframe():
    alarms = sorted(alarm_manager.get_all_alarms(), key=lambda x: x.get("time", "")); display_data = [{"ID": a.get("id"), "状態": a.get("enabled", False), "時刻": a.get("time"), "曜日": ",".join([DAY_MAP_EN_TO_JA.get(d, d.upper()) for d in a.get('days', [])]), "キャラ": a.get("character"), "テーマ": a.get("theme")} for a in alarms]
...
def toggle_selected_alarms_status(selected_ids: list, target_status: bool):
    if not selected_ids: gr.Warning("状態を変更するアラームが選択されていません。")
        ...
        changed_count = 0; status_text = "有効" if target_status else "無効"
...
def load_alarm_to_form(selected_ids: list):
    ...
    h, m = alarm.get("time", "08:00").split(":"); days_ja = [DAY_MAP_EN_TO_JA.get(d.lower(), d.upper()) for d in alarm.get("days", [])]
...
def handle_add_or_update_alarm(editing_id, h, m, char, theme, prompt, days):
    ...
    if success: df = render_alarms_as_dataframe(); return df, df, "アラーム追加", "", "", default_char, list(DAY_MAP_EN_TO_JA.values()), "08", "00", None
    df = render_alarms_as_dataframe(); return df, df, "アラーム更新" if editing_id else "アラーム追加", theme, prompt, char, days, h, m, editing_id
...
def update_token_count(textbox_content: Optional[str], file_input_list: Optional[List[Any]], current_character_name: str, current_model_name: str, current_api_key_name_state: str, api_history_limit_state: str, send_notepad_state: bool, notepad_editor_content: str, use_common_prompt_state: bool) -> str:
    if not all([current_character_name, current_model_name, current_api_key_name_state]): return "入力トークン数 (設定不足)"
    parts_for_api = [];
