...
def handle_location_change(character_name: str, location_id: str, api_key_name: str) -> Tuple[str, str]:
    ...
    world_settings_path = get_world_settings_path(character_name)
    world_data = load_memory_data_safe(world_settings_path)
    current_loc_name = world_data.get(current_loc_id, {}).get("name", current_loc_id) if "error" not in world_data else current_loc_id
    if not character_name or not location_id:
        gr.Warning("キャラクターと移動先の場所を選択してください。"); return current_loc_name, "（場所の変更に失敗しました）"
