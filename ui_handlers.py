...
def _replace_path_for_gradio(path: Optional[str]) -> str:
    if not path:
        return ""
    # Gradioの /file= は絶対パスまたはGradio起動ディレクトリからの相対パスを期待する
    # ここでは ATTACHMENTS_DIR や generated_images が Gradio から直接提供されることを前提とする
...
def format_history_for_gradio_wrapper(chat_log_from_utils: List[Dict[str, str]]) -> List[List[Optional[str]]]:
    ...
    for i, message_pair in enumerate(formatted_history_input):
        ...
        if isinstance(assistant_message_initial, str) and assistant_message_initial.strip():
            ...
            if current_processing_for_assistant.strip() and not temp_message_after_th and "【Thoughts】" in current_processing_for_assistant:
            ...
            if current_processing_for_assistant.strip() and not temp_message_after_html_th and "<div class='thoughts'>" in current_processing_for_assistant:
            ...
            current_processing_for_assistant = temp_message_after_html_th

            print(f"DEBUG_FORMAT_WRAPPER: Entry {i} - Assistant message after all thought removals, before common processing: '{current_processing_for_assistant[:200]}...'")

            original_segment_for_warning_check = current_processing_for_assistant

            modified_segment_step1_assistant = re.sub(FILE_CONTENT_HEADER_PATTERN, r"添付ファイル: \1", current_processing_for_assistant, flags=re.DOTALL)
            if current_processing_for_assistant != modified_segment_step1_assistant: print(f"DEBUG_FORMAT_WRAPPER: Entry {i} (Assistant) - After FILE_CONTENT_HEADER_PATTERN: '{modified_segment_step1_assistant[:200]}...'")
            ...
            current_processing_for_assistant = modified_segment_step1_assistant

            modified_segment_step2_assistant = re.sub(ATTACHED_IMAGE_PATTERN, lambda m: f"![\{_sanitize_alt_text(m.group(2))}]({_replace_path_for_gradio(m.group(1))})", current_processing_for_assistant)
            if current_processing_for_assistant != modified_segment_step2_assistant: print(f"DEBUG_FORMAT_WRAPPER: Entry {i} (Assistant) - After ATTACHED_IMAGE_PATTERN: '{modified_segment_step2_assistant[:200]}...'")
            if current_processing_for_assistant.strip() and not modified_segment_step2_assistant.strip() and "[ファイル添付:" in current_processing_for_assistant : print(f"DEBUG_FORMAT_WRAPPER: WARNING! Entry {i} (Assistant) - Segment became EMPTY after ATTACHED_IMAGE_PATTERN.")
            current_processing_for_assistant = modified_segment_step2_assistant

            modified_segment_step3_assistant = re.sub(GENERATED_IMAGE_PATTERN, lambda m: f"![generated_image]({_replace_path_for_gradio(m.group(1))})", current_processing_for_assistant)
            if current_processing_for_assistant != modified_segment_step3_assistant: print(f"DEBUG_FORMAT_WRAPPER: Entry {i} (Assistant) - After GENERATED_IMAGE_PATTERN: '{modified_segment_step3_assistant[:200]}...'")
            if current_processing_for_assistant.strip() and not modified_segment_step3_assistant.strip() and "[Generated Image:" in current_processing_for_assistant: print(f"DEBUG_FORMAT_WRAPPER: WARNING! Entry {i} (Assistant) - Segment became EMPTY after GENERATED_IMAGE_PATTERN.")
            final_assistant_message = modified_segment_step3_assistant

            print(f"DEBUG_FORMAT_WRAPPER: Entry {i} (Assistant) - Final value for segment: '{final_assistant_message[:200]}...'")
...
def update_ui_on_character_change(
    ...
    if log_f and os.path.exists(log_f):
        # ログファイルが存在する場合のみ読み込む
        chat_history_for_gradio = format_history_for_gradio_wrapper( # Use wrapper
...
def reload_chat_log(character_name: Optional[str]) -> List[List[Optional[str]]]:
    ...
    chat_log_for_display = format_history_for_gradio_wrapper( # Use wrapper
