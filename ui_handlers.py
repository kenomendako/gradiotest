# ui_handlers.py の全内容を、以下のコードで完全に置き換えてください
...
import locale
import subprocess
from typing import List, Optional, Dict, Any, Tuple, Iterator # Iteratorを追加
...
#
# --- メッセージ送受信・再生成ハンドラ (v3: タイムスタンプFIX) ---
#
    ...
    if file_input_list:
        for file_obj in file_input_list:
            ...
            if isinstance(file_obj, str):
                log_message_parts.append(f"[ペーストされたテキスト添付: {len(file_obj)}文字]")
            else: # ファイルオブジェクトの場合
...
def handle_context_settings_change(room_name: str, api_key_name: str, api_history_limit: str, add_timestamp: bool, send_thoughts: bool, send_notepad: bool, use_common_prompt: bool, send_core_memory: bool, send_scenery: bool):
    ...
    return gemini_api.count_input_tokens(
        ...
        api_history_limit=api_history_limit, **effective_settings
    )
    ...
    if file_list:
        for file_obj in file_list:
            try:
                # ▼▼▼【修正点3】データ型で処理を分岐 ▼▼▼
                if isinstance(file_obj, str):
                    # 文字列の場合は、その内容を直接トークン計算の対象にする
                    parts_for_api.append(file_obj)
                else:
                    # ファイルオブジェクトの場合は、パスを取得して処理
                    file_path = file_obj.name
                    file_basename = os.path.basename(file_path)
                    kind = filetype.guess(file_path)
                    if kind and kind.mime.startswith('image/'):
...
# ... (以降の関数は、handle_scenery_refresh から最後まで、変更ありません) ...
...
def handle_create_room(new_room_name: str, new_user_display_name: str, initial_system_prompt: str):
    ...
    try:
        ...
        manage_dd = gr.update(choices=updated_room_list, value=safe_folder_name)
...
def handle_save_room_config(folder_name: str, room_name: str, user_display_name: str, agent_display_name: str, description: str):
    ...
    try:
        ...
        with open(config_path, "r+", encoding="utf-8") as f:
            ...
            config["agent_display_name"] = agent_display_name.strip()
...
def handle_alarm_selection_for_all_updates(evt: gr.SelectData, df_with_id: pd.DataFrame):
    ...
    all_rooms = room_manager.get_room_list_for_ui()
    default_room_folder = all_rooms[0][1] if all_rooms else "Default"
    if len(selected_ids) == 1:
        ...
        if alarm:
            ...
            form_updates = (
                "アラーム更新", alarm.get("context_memo", ""), alarm.get("character", default_room_folder),
                days_ja, alarm.get("is_emergency", False), h, m, selected_ids[0]
            )
            cancel_button_visibility = gr.update(visible=True)
        else:
            form_updates = ("アラーム追加", "", default_room_folder, [], False, "08", "00", None)
...
def handle_cancel_alarm_edit():
    ...
    default_room_folder = all_rooms[0][1] if all_rooms else "Default"
    return (
        "アラーム追加", "", gr.update(choices=all_rooms, value=default_room_folder),
...
def handle_save_gemini_key(key_name, key_value):
    ...
    return gr.update(choices=new_keys)
