...
def handle_item_selection(world_data: Dict, area_id: str, room_id: Optional[str]):
    ...
    if area_id and room_id:
        ...
        return gr.update(choices=room_choices, value=None), gr.update(value="← 左のパネルからエリアや部屋を選択してください。", visible=True), gr.update(visible=False), gr.update(visible=False)
    return gr.update(choices=room_choices, value=room_id), gr.update(value=generate_details_markdown(selected_data), visible=True), gr.update(visible=False), gr.update(visible=True)
...
def handle_edit_button_click(world_data: Dict, area_id: str, room_id: Optional[str]):
    ...
    return gr.update(visible=False), gr.update(visible=True), gr.update(value=convert_data_to_yaml_str(selected_data))
...
def handle_save_button_click(character_name: str, world_data: Dict, area_id: str, room_id: Optional[str], editor_content: str):
    ...
    try:
        ...
        return world_data, gr.update(value=generate_details_markdown(new_data), visible=True), gr.update(visible=False)
