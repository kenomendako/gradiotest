...
def handle_message_submission(*args: Any):
    ...
    if not user_prompt_from_textbox and not file_input_list:
        ...
        yield chatbot_history, gr.update(), gr.update(), token_count, gr.update()
        return
    ...
    yield chatbot_history, gr.update(value=""), gr.update(value=None), token_count, gr.update()

    final_response_text = ""
    latest_scenery = "（情景描写は更新されませんでした）"
    try:
        invocation_result = gemini_api.invoke_nexus_agent(*args)
        final_response_text = invocation_result.get("response", "（応答エラー）")
        latest_scenery = invocation_result.get("scenery", "（情景エラー）")
    ...
    yield chatbot_history, gr.update(), gr.update(value=None), token_count, latest_scenery
...
def update_api_history_limit_state_and_reload_chat(limit_ui_val: str, character_name: Optional[str]):
    ...
    chat_history, _ = reload_chat_log(character_name, key)
    return key, chat_history, gr.State()
...
def get_current_location_info(character_name: str) -> (str, str):
    """現在の場所のIDと表示名を取得する。"""
    if not character_name:
        return "（不明）", "（不明）"
    ...
    location_details = living_space.get(location_id, {})
    display_name = location_details.get("name", location_id)

    return location_id, display_name

def update_location_ui(character_name: str, *agent_args):
    """場所関連UIをまとめて更新する。AIに情景描写を問い合わせる。"""
    args_list = list(agent_args)
    args_list[0] = "__get_scenery__"

    invocation_result = gemini_api.invoke_nexus_agent(*args_list)
    scenery = invocation_result.get("scenery", "（情景描写の取得に失敗しました）")

    location_id, display_name = get_current_location_info(character_name)
    locations = get_location_list_for_ui(character_name)

    return gr.update(choices=locations, value=location_id), display_name, scenery

def handle_initial_load(char_name_to_load: str, *args):
    """アプリケーション起動時にUIの全要素を一度に初期化する司令塔関数。"""
    (api_history_limit, send_notepad_state, use_common_prompt_state,
     add_timestamp_state, send_thoughts_state, send_core_memory_state,
     send_scenery_state, api_key_name) = args
    ...
    current_location_id, display_name = get_current_location_info(returned_char_name)
    scenery = "（チャットを開始するか、下のボタンで更新してください）"

    initial_token_str = update_token_count(
        None, None, returned_char_name, config_manager.initial_model_global,
        api_key_name, api_history_limit,
        send_notepad_state, use_common_prompt_state,
        add_timestamp_state, send_thoughts_state, send_core_memory_state,
        send_scenery_state
    )
