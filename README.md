...
1.  **必要なものをインストール:**
    - `requirements.txt`を開き、指示に従ってライブラリをインストールします。
    - `python -m spacy download ja_core_news_lg` を実行します。
2.  **起動:**
    - このフォルダにある`ネクサスアーク.bat`をダブルクリックしてアプリを起動します。
    - 黒いウィンドウに表示されるURL (`http://127.0.0.1:7860`) をブラウザで開きます。
3.  **APIキーを設定:**
    - アプリ画面左側の「⚙️ 設定」を開きます。
    - 「共通」タブの中にある「🔑 APIキー / Webhook管理」を開きます。
    - 「Gemini APIキー」の欄に、あなたのGoogle AI StudioのAPIキーを入力して「保存」ボタンを押します。
4.  **チャット開始！**
    - これで準備完了です。AIとの対話をお楽しみください。
