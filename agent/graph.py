...
def get_configured_llm(model_name: str, api_key: str):
    return ChatGoogleGenerativeAI(
        ...
        convert_system_message_to_human=False, max_retries=6
    )

# --- 5. context_generator_node (安定版) ---
def context_generator_node(state: AgentState):
    # (この関数は安定版のため、内容は変更ありません)
    if not state.get("send_scenery", True):
        ...
        prompt_vars = { 'character_name': state['character_name'], 'character_prompt': character_prompt, 'core_memory': core_memory, 'notepad_section': notepad_section, 'tools_list': tools_list_str }
    ...
    scenery_text = "（現在の場所の情景描写は、取得できませんでした）"; space_def = "（現在の場所の定義・設定は、取得できませんでした）"; location_display_name = "（不明な場所）"
    try:
        location_id_to_process = None
        last_tool_message = next((msg for msg in reversed(state['messages']) if isinstance(msg, ToolMessage) and "Success: Current location has been set to" in msg.content), None)
...
print("--- [最終FIX] 安全なツール実行機能を持つグラフがコンパイルされました ---")
