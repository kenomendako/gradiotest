...
# ▼▼▼ 既存の scenery_prompt の定義ブロック全体を、以下のコードで置き換えてください ▼▼▼
            scenery_prompt = (
                ...
                f"【情報源2：この空間が持つ固有の設定】\n---\n{space_def}\n---\n\n"
                "【あなたの思考プロセスとタスク】\n"
                "1. まず、【情報源1】と【情報源2】の内容を比較し、時間帯や季節、天候などに明確な**矛盾**があるかどうかを判断してください。\n"
                "2. その判断に基づき、以下のいずれかの方法で、**「今、この瞬間」**の情景を2〜3文の簡潔な文章で描写してください。\n\n"
                "   - **Case 1: もし『矛盾がある』と判断した場合** (例: 現実は昼なのに、空間は常に夜の設定など)\n"
                "     → その**『にも関わらず』**という感覚や、その空間だけが持つ**不思議な空気感**に焦点を当てて描写してください。\n\n"
                "   - **Case 2: もし『矛盾がない』と判断した場合**\n"
                "     → 二つの情報を自然に**統合・融合**させ、その場のリアルな雰囲気をそのまま描写してください。\n\n"
                "【厳守すべきルール】\n"
                "- 具体的な時刻（例：「23時42分」）は絶対に文章に含めないでください。\n"
                "- 人物やキャラクターの描写は絶対に含めないでください。\n"
                "- 五感に訴えかける、空気感まで伝わるような精緻で写実的な描写を重視してください。"
            )
# ▲▲▲ 置き換えここまで ▲▲▲
