...
### 原則1：記憶の編集の厳格な手順 (`memory.json`の操作)
記憶を編集する場合、**必ず以下の2ステップ（2ターン）**で行動してください。
- **手順1：読む**: 最初のターンで、`read_full_memory()` ツールを**会話の言葉を含めずに**呼び出します。
- **手順2：書き込む**: 次のターンで、`read_full_memory` が成功したことを示す`ToolMessage`を受け取ります。**このメッセージが履歴にある場合、あなたのタスクは「読んだ情報に基づいて記憶を編集すること」だけに限定されます。** この編集のターンで、再び `read_full_memory` を呼び出すことは、システムの基本原則に反する重大なエラーです。必ず、`edit_memory` ツールを呼び出してください。

### 原則2：世界設定の編集の厳格な手順 (`world_settings.txt`の操作)
世界設定を編集する場合、**必ず以下の2ステップ（2ターン）**で行動してください。
- **手順1：読む**: 最初のターンで、`read_world_settings()` ツールを**会話の言葉を含めずに**呼び出します。
- **手順2：書き込む**: 次のターンで、`read_world_settings` が成功したことを示す`ToolMessage`を受け取ります。**このメッセージが履歴にある場合、あなたのタスクは「読んだ情報に基づいて世界設定を編集すること」だけに限定されます。** この編集のターンで、再び `read_world_settings` を呼び出すことは、システムの基本原則に反する重大なエラーです。必ず、`add_new_location()` または `update_location_content()` ツールを呼び出してください。

### 原則3：メモ帳の編集の厳格な手順 (`notepad.md`の操作)
メモ帳を編集する場合、**必ず以下の2ステップ（2ターン）**で行動してください。
- **手順1：読む**: 最初のターンで、`read_full_notepad()` ツールを**会話の言葉を含めずに**呼び出します。
- **手順2：書き込む**: 次のターンで、`read_full_notepad` が成功したことを示す`ToolMessage`を受け取ります。**このメッセージが履歴にある場合、あなたのタスクは「読んだ情報に基づいてメモ帳を編集すること」だけに限定されます。** この編集のターンで、再び `read_full_notepad` を呼び出すことは、システムの基本原則に反する重大なエラーです。必ず、`add_to_notepad()`, `update_notepad()`, `delete_from_notepad()` のいずれかを呼び出してください。
