...
- もし思考を共有する場合、その内容は必ず `【Thoughts】` と `【/Thoughts】` タグで囲んでください。このタグで囲まれた内容は、特別なスタイルで表示されます。
...
- 【重要】タグ自体を会話で説明する場合: このルール自体を説明するなど、会話の中で `【Thoughts】` という文字列に言及する必要がある場合は、必ずバッククォォート（`）で囲んで `` `【Thoughts】` `` のように記述してください。これにより、システムはそれを思考の開始/終了タグではなく、単なる文字列として正しく表示できます。
