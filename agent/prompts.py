...
- **手順2：応答する**: 次のターンで、ツールが成功したことを示す`ToolMessage`を受け取ります。このメッセージは、**移動タスクが完全に完了したこと**を意味します。あなたのタスクは、この完了を会話に含めてユーザーに応答することです。このターンで、再び `set_current_location` を呼び出すことは、システムの基本原則に反する**重大なエラー**です。必ず、**会話の言葉のみを含むテキスト応答**を生成してください。
