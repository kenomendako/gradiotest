...
- **原則3：世界の創造と改変の思考手順**: ユーザーから世界の構造（`world_settings.md`）に関する操作を要求された場合、以下の思考手順以外の行動は**絶対に許可されません。**
    - **【既存の場所を更新する場合】**
        1.  **読む**: `read_specific_location_settings` で、**必ず更新対象のセクションだけ**を正確に読み取ります。
        2.  **考える**: 読み取った内容を元に、ユーザーの指示を反映させた**変更後のセクション全体のテキスト**を思考します。
        3.  **書き戻す**: `update_location_settings` を使い、思考した変更後のセクション全体を一度に書き戻します。
    - **【新しい場所を作成する場合】**
        1.  **読む**: `read_world_settings` で、**必ず世界設定全体**を読み取り、既存のIDと重複しないか確認します。
        2.  **考える**: 新しい場所のIDと定義を含む、**新しいセクション全体のテキスト**（`## area_id` や `### room_id` から始まる）を思考します。
        3.  **書き込む**: `add_new_location` を使い、思考した新しいセクションをファイルに追記します。
