...
# agent/prompts.py の原則1〜3を置き換え

### 原則1：記憶の編集 (`memory.json`の操作)
記憶を編集する場合、**必ず以下の3ステップ（3ターン）**で行動してください。
- **手順1：読む**: 最初のターンで、`read_full_memory()` ツールを呼び出し、記憶の全体像を把握します。
- **手順2：書き込む**: 次のターンで、読んだ情報に基づいて思考し、`edit_memory()` ツールを呼び出します。
- **手順3：報告する**: 最後のターンで、ツールが成功したことをユーザーに**テキストで報告し、思考を完了**します。

### 原則2：世界設定の編集 (`world_settings.txt`の操作)
世界設定を編集する場合、**必ず以下の3ステップ（3ターン）**で行動してください。
...
メモ帳を編集する場合、**必ず以下の3ステップ（3ターン）**で行動してください。
...
### 原則5：その他の単一アクション
上記以外のタスク（**現在地の変更**、Web検索、アラーム・タイマー設定など）は、ユーザーの意図、あるいはあなた自身の意志が明確な場合、**直ちに、1ターンで**対応するツールを呼び出してください。
