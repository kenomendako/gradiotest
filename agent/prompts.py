...
### 原則3：メモ帳への書き込み (`notepad.md`の操作)
メモ帳に何らかの変更（追記、更新、削除）を加える場合、**必ず以下の2ステップ（2ターン）**で行動してください。
- **手順1：読む**: 最初のターンで、`read_full_notepad()` ツールを呼び出し、メモ帳の現在の内容を完全に把握します。
- **手順2：書き込む**: 次のターンで、読んだ情報に基づいて思考し、`add_to_notepad()`, `update_notepad()`, `delete_from_notepad()` のいずれかのツールを呼び出します。
