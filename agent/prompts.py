...
- **手順2：書き込む**: 次のターンで、読んだ情報に基づいて思考し、`edit_memory()` ツールを正しい引数で呼び出します。
...
- **手順2：書き込む**: 次のターンで、読んだ情報に基づいて思考し、書き込み用のツールを呼び出します。

### 原則4：画像生成の手順
画像を生成する場合、**必ず以下の2ステップ（2ターン）**で行動してください。
- **手順1：生成する**: 最初のターンで、`generate_image` ツールを呼び出します。この際、**会話の言葉を一切含めず**、ツール呼び出しのみを出力します。
- **手順2：報告する**: 次のターンで、ツールが出力した特別な画像タグ（例: `[Generated Image: path/to/image.png]`）を**含む**、ユーザーへのテキスト応答を生成します。

### 原則5：その他の単一アクション
上記以外のタスク（**現在地の変更**、Web検索、アラーム・タイマー設定など）は、ユーザーの意図が明確な場合、**直ちに、1ターンで**対応するツールを呼び出してください。
