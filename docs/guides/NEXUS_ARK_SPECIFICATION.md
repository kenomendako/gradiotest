...
左カラムの「🏠 チャットルームの作成・管理」メニューから行います。
...
- **堅牢な保存システム:** 全ての設定ファイル（`config.json`, `room_config.json`）の保存時に適用されます。
  - **自動バックアップ:** 保存直前に、現在の内容を `backups/` フォルダ内にタイムスタンプ付きでコピーします。
  - **アトミック書き込み:** 一時ファイル（`.tmp`等）に書き込んだ後、OSレベルの `replace` 操作で元のファイルを置き換えることで、保存中のクラッシュによるファイル破損を防ぎます。
...
**ツール使用時のみ**、AIは「〜しますね」のような意思表示（第一幕）と、ツール実行後の最終報告（第二幕）を分けて応答します。ツールを使用しない通常の会話は、1ループで即終了します（APIコスト削減のため）。
...
### 10.4. 想起システム (Pre-response Retrieval)
ユーザーからの入力に対し、AIが応答を生成する**前**に、自律的に関連情報を検索・想起する機能です。

*   **プロセス:**
    1.  **`retrieval_node`**: ユーザーの入力内容を高速モデル（`gemini-2.5-flash-lite`）が分析し、検索が必要か判断します。
    2.  **クエリ生成**: 必要と判断した場合、ユーザーの言葉から「連想されるキーワード」を複数生成します（例：「娘の主治医」→「娘 病院 先生 医師...」）。
    3.  **マルチソース検索**:
        *   知識ベース（RAG）
        *   過去の会話ログ（キーワードOR検索）
        *   日記（Memory）
    4.  **遅延注入**: 検索結果は、`agent_node`の実行直前にシステムプロンプトの `{retrieved_info}` セクションに動的に挿入されます。
...
- **room_config.json**: ルームの表示名や個別設定などを保存するファイルです。保存時に `backups/configs/` へ自動バックアップが作成されます。
