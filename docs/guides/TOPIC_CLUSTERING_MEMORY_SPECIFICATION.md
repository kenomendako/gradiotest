...
> [!WARNING]
> **2024-12-28 一時無効化**: 現在、`retrieval_node`での話題クラスタ検索は一時無効化されています。
> クラスタリング精度（ラベルの抽象性、要約の不十分さ）を改善するタスクが完了後、再有効化される予定です。
> 睡眠時記憶整理での話題クラスタ更新機能は引き続き動作します。
...
### 3.1 ハイブリッド・クラスタリング・プロセス
1. **ベクトル化**: 全てのエピソード記憶を `EmbeddingProvider` で多次元ベクトルに変換。
2. **固定トピック割り当て (教示的分類)**:
    - ユーザーが「固定トピック」を指定している場合、HDBSCANの前に実行される。
    - 各エピソードに対し、固定トピック名とのセマンティック類似度を計算。
    - 閾値（内部設定）を超えたエピソードは、HDBSCANを介さず対応する固定トピックに強制分類される。
3. **HDBSCAN (自律的分類)**:
    - 固定トピックに分類されなかった残りのエピソードに対し、密度ベースのクラスタリングを実行。
    - 関連性の高いエピソードをグループ化し、新しい「未知の話題」を見つけ出す。
4. **AIラベリングと要約**:
    - HDBSCANで生成された各クラスタに対し、LLM（Gemini-2.5-flash）が最適なタイトルと概要を生成。
    - 固定トピックについては、既存のトピック名と文脈を維持しつつ、最新の内容を要約に反映。
