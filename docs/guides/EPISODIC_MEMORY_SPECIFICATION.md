...
# 追記
    *   **エラー時の挙動:** コンテンツポリシー（PROHIBITED_CONTENT）等で生成がブロックされた場合でも、「（エラーにより要約できませんでした）」という記録を残して保存する。これにより、次回実行時に同じ日の処理で無限ループに陥ることを防ぐ。
...
## 6. 関連ファイル
*   `episodic_memory_manager.py`: 要約生成、JSON読み書き、フィルタリングロジックの中核。
*   `ui_handlers.py`: ボタンイベントの制御、UIロック機構。
*   `agent/graph.py`: プロンプトへの注入ロジック（実際の思考時）。
