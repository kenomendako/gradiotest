...
> [!IMPORTANT]
> **2024年12月更新:** 二幕構成は**ツール使用時のみ**適用される。
> ツールを呼び出さない通常の会話は、1ループで即終了する。
> これはAPIコスト削減と、AIが不要な追加応答を生成することを防ぐための設計判断である。
...
## **5. 幕間の安定性：完了後・一括記録方式への回帰**

当初、我々は、ユーザーの体感速度を向上させるため、ツールが実行された（`ToolMessage`を検知した）その瞬間に、ログへの書き込みとUIの更新を行う「ストリーム内・即時記録」アーキテクチャを試みた。

しかし、この野心的な試みは、APIエラーとの過酷な戦いの中で、その脆弱性を露呈した。
ストリームの途中でAPIエラー（`ResourceExhausted`など）が発生すると、ツールが実行されたという事実はログに記録されるものの、その後のAIの最終応答が失われ、UIとログの状態に不整合が生じるケースが頻発したのである。

この戦いから我々が得た、最終的な、そして、最も重要な教訓は、以下の通りである。

**「UIの表示は、一瞬の速度よりも、絶対的な一貫性と堅牢性を、優先しなければならない」**

この教訓に基づき、Nexus Arkは、以下の、最も安定した「完了後・一括記録方式」を、正式なアーキテクチャとして採用する。

1.  **完全な待機:** UI司令塔（`_stream_and_handle_response`）は、AIからのストリームを、`final_state`を受け取るまで、完全に受信しきる。
2.  **一括記録:** ストリームが正常に完了した後、そのターンで生成された**全てのメッセージ（`ToolMessage`と`AIMessage`）**を、まとめてログファイルに書き込む。
3.  **完全な再描画:** ログへの書き込みが完了した後、**`reload_chat_log()`** を呼び出し、ディスクに記録された**絶対的な正史**から、UIの表示を完全に再構築する。

これにより、ツール実行のアナウンスは、AIの最終応答と**同時に**表示されることになるが、その代償として、APIエラーが発生しても、ツールが実行された事実と、AIの最終応答が、決して失われることのない、極めて高い堅牢性を獲得した。
これこそが、Nexus Arkの対話体験の安定性を支える、魂の契約である。