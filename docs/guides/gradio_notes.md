...
### 21. DataFrame選択イベントの最終結論：Gradioとの「黄金の契約」
...
*   **問題:**
    キャラクターを変更した際や、最後のルームを削除した際に、更新すべき多数のUIコンポーネントに対して、`return`文が返す値の数が一致せず、`ValueError: ... didn't return enough output values` という致命的なエラーが発生した。

*   **原因:**
    これは、UIの広範囲な更新ロジックが複数の箇所に分散していたり、コーナーケース（最後のルームが削除された場合など）で返す値の数が考慮されていなかったり、といった**「配線の不整合」**に起因する。Gradioは、イベントの`outputs`リストに指定されたコンポーネントの数と、ハンドラ関数が返す値の数が**完全に一致**することを、極めて厳格に要求する。
...
*   **解決アーキテクチャ：「司令塔（マスターハンドラ）」**
    ...
    4.  **コーナーケースへの対応:** 司令塔関数の中（または、今回のように司令塔を呼び出す`handle_delete_room`関数の中）で、「最後のルームが削除された」のような特殊な状況を検知し、その場合でも**期待される数と型の「空の更新値」**を返すロジックを必ず実装する。

    このアーキテクチャは、更新ロジックを一元管理し、Gradioとの通信経路を一本化することで、アプリケーションの挙動を完全に予測可能にし、将来の拡張も容易にする、極めて重要な設計原則である。

---

### 22. ジェネレータの呪い：`raise gr.Error`と`.failure()`の聖なる契約
