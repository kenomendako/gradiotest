...
*   **問題:**
    キャラクターを変更した際や、ワールド・ビルダーで世界設定を保存した際に、更新すべき多数のUIコンポーネントに対して、ハンドラ関数が返す値の数が一致せず、`ValueError: ... didn't return enough output values` や `UserWarning: ... returned too many output values` という致命的、あるいはデバッグ困難なエラーが頻発した。
*   **原因:**
    これは、一つのイベント（キャラクター変更など）が、本来**同時に更新されるべき**複数のUI群（チャットタブのコンポーネント、ワールド・ビルダータブのコンポーネントなど）に対して、複数の、あるいは矛盾した「配線図」を持ってしまっていたことに起因する。Gradioは、イベントの`outputs`リストに指定されたコンポーネントの数と、ハンドラ関数が返す値の数が**完全に一致**することを、極めて厳格に要求するため、このような複雑な依存関係を安定して解決することが苦手な場合がある。
