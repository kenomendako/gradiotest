...
### 18. Markdown内戦：ログ解析とHTMLレンダリングを完全統一する

*   **問題:**
    AIとの対話でMarkdown形式のテキスト（特に `##` を含む見出しや ` ``` ` を含むコードブロック）を共有すると、以下の複数の問題が同時に発生し、チャット表示が完全に崩壊した。
    1.  **話者の誤認:** 本文中の `##` が話者の切り替えと誤認され、一つのメッセージが複数に分割された。
    2.  **HTMLタグの露出:** コードブロックが整形されず、`<br>` のようなHTMLタグがそのまま表示された。
    3.  **UI要素の消失:** 以前のバージョンでは表示されていた「話者名」や「ナビゲーションボタン（▲▼）」が表示されなくなった。
    4.  **レイアウト崩壊:** コードブロックがコンテナの幅を無視してはみ出し、強制的に横スクロールが発生した。

*   **原因の探求：三つの罪の連鎖**
    この問題は、単一のバグではなく、過去のバグ修正が積み重なって生まれた、アーキテクチャ上の「内戦」であった。
    1.  **第一の罪（過剰な防衛）:** 過去に`<pre>`タグの横スクロール問題（教訓11）を回避するため、GradioのMarkdownレンダラを信用せず、**すべての改行を手動で`<br>`に置換する**という安全策が取られていた。これが、コードブロック内の改行まで破壊する副作用を生んだ。
    2.  **第二の罪（単純な解析）:** ログ解析ロジック(`utils.py`)が、単純に「`## `で始まる行」を話者の区切りと見なしていたため、メッセージ本文中のMarkdown見出しを誤認した。
    3.  **第三の罪（中途半端な信頼）:** これらの問題を解決しようとして、一時的にGradioのレンダラを信頼する方針に切り替えた。しかし、これにより、話者名やナビボタンを付与する**カスタムHTMLの生成ロジックが失われ**、UI要素が消失した。

*   **解決アーキテクチャ：「ハイブリッド・レンダリング」による完全掌握**
    この内戦を終結させるための最終的な解決策は、Gradioのレンダラに任せるのでも、完全な手動レンダリングでもない、両者の長所を組み合わせた「ハイブリッド・レンダリング」である。

    1.  **第一の柱：不壊のログパーサー (`utils.py`)**
        *   ログファイルを単純に分割するのではなく、正規表現の`finditer`を用いて、まず**全ての有効なヘッダー（`## ROLE:NAME`）の位置を特定**する。
        *   そして、あるヘッダーから次のヘッダーまでの間の全てのテキストを、一つのメッセージ内容として正確に切り出す。
        *   これにより、メッセージ本文にいかなるMarkdown構文が含まれていても、話者が誤認されることは**絶対に**なくなる。

    2.  **第二の柱：サーバーサイド・レンダリング (`ui_handlers.py`)**
        *   GradioにMarkdownを渡す前に、Python側で`markdown-it-py`ライブラリを使い、メッセージ内容を**先に完全なHTMLに変換**する。これにより、コードブロックは正しく`<pre><code>`タグで囲まれる。
        *   この生成されたHTML文字列に、**話者名**や**ナビゲーションボタン**といった、我々がコントロールしたい付帯的なHTML要素を**結合**する。
        *   この「完成品のHTML」をGradioに渡すことで、表示の曖昧さを完全に排除する。

    3.  **第三の柱：テーマ追従型CSSによる最終仕上げ (`nexus_ark.py`)**
        *   Gradioのテーマ変数 (`var(--background-fill-secondary)`など) を活用したカスタムCSSを定義する。
        *   このCSSが、上記プロセスで生成された`<pre>`タグと`<code>`タグに対して、**強制的な折り返し**と、**現在のテーマに合わせた美しいスタイリング**を適用する。

    この三位一体のアーキテクチャは、ログデータの解釈から最終的なピクセル表示に至るまで、アプリケーションが表示を完全に掌握し、安定性と表現力を両立させるための、Nexus Arkにおける最終的な答えである。