...
- `ui_handlers.py`: タイムスタンプ追加処理・AI模倣除去ロジック（L1115-1138）
...
#### 16. スマホ等縦長画面でのサイドバー・スクロールの聖域 (2025-12-29)
*   **問題:**
    スマホなどの縦長画面（各カラムが縦に積み重なるレスポンシブ表示）において、メインエリア（右カラム）やオーバーレイ表示されたサイドバーが正常にスクロールできず、下部のメニューやボタンに到達できない問題が発生した。
*   **原因:**
    GradioのデフォルトCSS、またはカスタムCSSで `height: 100vh !important` などをコンテナに強引に適用すると、中身が画面の高さを超えてもブラウザが「収まっている」と判断し、スクロールバーを生成しなくなるため。また、モバイルでのサイドバーは `position: absolute` で重なる挙動が標準だが、これの `height` 設定を誤ると操作不能になる。
*   **解決アーキテクチャ：「dvh と overflow-y の適切な配分」**
    1.  **最上位コンテナ (`.gradio-container`):** モバイル表示時 (`@media (max-width: 768px)`) は `height: auto !important` および `min-height: 100dvh` を設定し、コンテンツ量に応じて全体が伸びるようにする。
    2.  **サイドバー (`div.sidebar`):** `height: 100dvh !important` を維持しつつ、**内部コンテナ (`div.sidebar-container`)** に対してのみ `overflow-y: auto !important` を適用する。これにより、オーバーレイとしての挙動（画面を覆う）を維持したまま、中身だけをスクロール可能にする。
    3.  **予備の余白:** スマホのブラウザUI（下部のアドレスバー等）との干渉を避けるため、スクロール領域の最下部には `padding-bottom: 100px` 程度の十分なマージンを確保するのがNexus Arkの流儀である。
*   **教訓:**
    「画面いっぱいに表示したい」という意図で `100vh` を多用すると、モバイルでのスクロールを殺す。常に `overflow-y: auto` がどのレイヤーで効いているかを意識し、コンテナを突き抜けるコンテンツの逃げ道を確保せよ。

---

### レッスン34: チャットログのシステムメッセージ形式と吹き出しの独立性 (2025-12-29)

#### 問題の症状

インポート機能で `## SYSTEM\n--- 会話開始 ---` という形式のメッセージをログに挿入したところ、チャット画面上で独立した「システムの吹き出し」として表示されず、直前のユーザーまたはAIの発言と同じ吹き出しの中に吸収されて表示される現象が発生した。

#### 根本原因

Nexus Arkのログパーサー（`format_history_for_gradio`）およびGradioのレンダリングにおいて、システムメッセージとして正しく「開始」を認識させるためには、以下の2つの条件が必要であることが判明した。

1.  **ヘッダー形式の厳格性**: 単なる `## SYSTEM` ではなく、`## SYSTEM:名前` のように、**コロンと識別子**を含める必要がある。これにより、パーサーが明確なシステムセクションの開始として認識できる。
2.  **改行によるセクション分離**: ヘッダーの直後に空行（`\n\n`）がない場合、Markdownの解釈やパースの過程で、直前のメッセージブロックの末尾として誤認される可能性がある。

#### 解決策

システムメッセージを生成する際は、以下の「黄金の形式」を遵守する。
...
1.  **標準形式への準拠**: プロジェクト内で既に使用されている形式（`## SYSTEM:tool_result` など）を常に参照し、独自の形式を導入する前に既存のパースロジックとの適合性を確認すること。
2.  **空行は安全装置**: 異なる役割を持つメッセージ（USER, AGENT, SYSTEM）を結合する際は、ヘッダーと本文の間、およびメッセージ同士の間に十分な空行を挿入することが、予期せぬ「吹き出しの吸収」を防ぐ最も確実な方法である。
