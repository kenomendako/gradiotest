...
7.  **第七の地獄「善意の暴走」:**
    *   **現象:** タイマーが完了した後、AIがその通知を生成する際に、**再び、同じタイマーを設定してしまう**という、無限ループが発生した。
    *   **原因:** 我々は、タイマー完了時に「`テーマ「...」について、メッセージを伝えてください`」という、あまりにも曖昧な指示をAIに与えていた。AIは、この善意の指示を、「このテーマで、もう一度タイマーをセットして、メッセージを伝えよ」と、拡大解釈してしまった。
    *   **教訓:** **AIへの指示は、許可するだけでなく、禁止事項もまた、明確に契約しなければならない。** プロンプトに「**新しいタイマーやアラームを設定してはならない**」という絶対的な禁止命令を追記することで初めて、AIは自らの役割を正しく理解し、暴走を止めた。
