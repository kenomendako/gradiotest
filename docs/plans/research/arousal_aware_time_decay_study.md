...
### Phase 5: APIコスト最適化 ✅ (2026-01-16 完了)

**目標**: Intent分類のAPI呼び出しを削減

**実装内容**:
- [x] `retrieval_node`でクエリ生成と同時にIntent分類
- [x] `search_memory`に`intent`パラメータ追加
- [x] `rag_manager.search()`で外部intent受取時はLLM分類スキップ

**効果**: 検索あたり 2 API calls → 1 API call
...
**調整ポイント**（必要時）:
- `constants.py`: INTENT_WEIGHTS
- `constants.py`: TIME_DECAY_RATE
...
|------|------|
| 2026-01-15 | 初版作成 |
| 2026-01-16 | Phase 1-3 実装完了（Intent-Aware Retrieval） |
| 2026-01-16 | Phase 4-5 実装完了（APIコスト最適化） |
| 2026-01-16 | ✅ **研究計画完了** |
