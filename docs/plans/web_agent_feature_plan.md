...
> **最終更新**: 2026-01-04
> **ステータス**: ✅ Phase 1 & 2 完了、Phase 3 待機中
...
| タスク | 内容 | 状態 |
|--------|------|------|
| 2.1 | `watchlist.json`スキーマ設計（ルームごと） | [x] |
| 2.2 | `watchlist_tools.py`新規作成 | [x] |
| 2.3 | キャッシュ機構（前回取得内容の保存） | [x] |
| 2.4 | 差分検出ロジック実装 | [x] |
| 2.5 | UIにウォッチリスト管理画面追加 | [x] |
| 2.6 | 監視頻度設定UI（指定時間/数時間おき/手動） | [x] |
| 2.7 | `alarm_manager.py`との統合（定期実行） | [x] |
...
| 日付 | 内容 |
|------|------|
| 2026-01-04 | **Phase 2 実装完了** - ウォッチリスト管理、UI、定期実行統合 |
