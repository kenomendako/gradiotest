...
- [ ] **研究・分析ノートの全文がシステムプロンプトに注入されている** 🆕
  - コンテキスト圧迫するので改善案を考えたい (INBOXより)

- [ ] **創作ノート改善** 🆕
  - また1行ごとにタイムスタンプ付くようになってしまっている。研究ノートも同じ仕様になってる可能性。エピソード記憶などのように1度の追加ごとに一つのタイムスタンプにして、一つずつ閲覧できるようにしたい。 (INBOXより)

- [ ] **英語対応** 🆕
  - メニューの日本語表示と英語表示を切り替えられるようにする。 (INBOXより)

- [ ] **トークン数カウントの見直し** 🆕
  - 本日分の指定文字数超過時の要約に置き換えた時にトークン数カウントが減ってない気がする。GeminiAPI不使用の一律文字数かTictokenでも良いから細かく反映させたい。ツールプロンプトをオンオフしても変わらない。その会話ターン終了後にでも記憶想起分も含めたトータル送信トークンも表示したい。 (INBOXより)
...
- [ ] **自律行動時の記憶想起をオフに** 🆕 (2026-01-03 INBOXより)
  - クエリ[SILENT]で検索しているので無駄
  - 自律行動時の送信コンテキストから記憶想起を除外する
...
- [ ] **Web巡回ツールのリスト改良** 🆕
  - ジャンル（AI技術・グルメ等）を指定したらAIがそのジャンルの巡回リストを作り巡回するようにしたい。リストは複数作成可能。 (INBOXより)

- [ ] **エンティティ記憶の定期統合・整理機能** 🆕
  - エンティティ記憶が追記され続け冗長化している。定期的にAIで統合・整理する仕組みが必要。 (INBOXより)
