...
- [ ] **情景画像送信プロンプトの最適化** 🆕
  - ペルソナがくどいくらい見えていることを話す問題をプロンプト調整で修正。
- [ ] **情景画像送信のデフォルト設定確認** 🆕
  - `enable_every_turn_vision` が初期状態でオフになっているか再確認。
- [ ] **APIキー設定の集約管理** 🆕
  - OpenAI互換プロバイダのAPIキー設定を「🔑 APIキー / Webhook管理」に集約。
- [ ] **OpenRouterエラー表示の修正** 🆕
  - ツール非対応モデル使用時にエラーメッセージが表示されない問題を修正。
- [ ] **動画アバターUIの表示不整合修正** 🆕
  - アバター設定が動画になっていてもUIでは「静止画」にチェックが入ってしまう問題を修正。
- [ ] **現在地移動ツールのメッセージクリーンアップ** 🆕
  - AIへのメタメッセージ（[RAW_RESULT]など）がチャットに表示されないように修正。
