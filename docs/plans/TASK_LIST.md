...
- [x] **Zhipu AI (GLM-4) の設定UI統合** ✅ (2026-01-28 完了)
  - 詳細: 共通設定・個別設定の「⚡ AIモデルプロバイダ設定」で Zhipu AI (GLM-4) を選択できるようにする。
  - 現状は内部処理モデルとしてのみ設定可能だが、会話用メインモデルとしても利用可能にする。
  - 優先度: 高
  - [レポート](../reports/2026-01-28_Zhipu_Integration_Fix.md)

- [x] **Zhipu AI ツール実行エラー (404) の修正** ✅ (2026-01-28 完了)
  - 詳細: ツール実行用LLM初期化時にルーム名が渡されず、Zhipu設定時でもGoogle APIを使用してしまう不具合を修正。
  - プロバイダ判定ロジックに room_name を正しく渡す。
  - [レポート](../reports/2026-01-28_Zhipu_Tool_Provider_Fix.md)
