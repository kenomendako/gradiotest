...
- [x] GLM-4対応（Zhipu AI） [DONE]
  - GLM-4-Flash/FlashX: **内部処理用**（要約、分類、軽量タスク）
  - GLM-4.7: **最終応答候補**（コスパ重視ユーザー向け選択肢）
- [x] APIキー UI の改善と集約
- [x] Groq対応（高速推論） [DONE 2026-01-24]
- [x] ローカルLLM対応（llama-cpp-python） [DONE 2026-01-24]
  - Ollama廃止、llama-cpp-pythonに統一（配布容易性のため）

#### Phase 4: フォールバック機構 [DONE 2026-01-24]
- [x] フォールバック有効/無効チェックボックス
- [x] `create_chat_model_with_fallback` ラッパー関数
- [ ] コスト最適化ダッシュボード（使用量可視化） → 将来検討
