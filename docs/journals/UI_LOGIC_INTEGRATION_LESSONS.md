...
## 教訓 43: 複数モジュール間のキャッシュ同期問題 (2026-01-05)

*   **発生した問題:**
    通常会話完了時に `MotivationManager.update_last_interaction()` で永続化ファイルを更新しても、`alarm_manager.py` の `check_autonomous_actions()` が古いクールダウン時間を表示し続けた。

*   **原因:**
    `alarm_manager.py`内にメモリ上のキャッシュ変数 `_last_autonomous_trigger_time` があり、クールダウン判定でこの**メモリキャッシュを優先して参照**していた。他のモジュール（`ui_handlers.py`）が永続化ファイルを更新しても、メモリキャッシュは更新されない。

    ```python
    # 問題のあったコード
    last_trigger = _last_autonomous_trigger_time.get(room_folder)  # キャッシュ優先
    if not last_trigger:
        last_trigger = mm.get_last_autonomous_trigger()  # ファイルは二次的
    ```

*   **解決策:**
    複数モジュールから更新される可能性がある値は、**キャッシュを優先するのではなく、常に永続化データ（Single Source of Truth）から読む**。

    ```python
    # 修正後のコード
    last_trigger = mm.get_last_autonomous_trigger()  # 常にファイルから読む
    ```

*   **教訓:**
    パフォーマンスのためのキャッシュは有用だが、**複数のモジュールから同じデータが更新される可能性がある場合は、キャッシュを使ってはならない**。または、キャッシュを使うなら、更新を通知するイベントシステム（Pub/Sub）を導入し、全モジュールのキャッシュを同期させる必要がある。

    今回のように「自分が更新したデータしか自分は見ない」という設計の場合、キャッシュは整合性を破壊する。**ディスク（ファイル）を Single Source of Truth として、全モジュールがそこから読む**設計が最も堅牢。

---

## 教訓 44: 関数実装と関数呼び出しは必ずセットで確認せよ (2026-01-05)

*   **発生した問題:**
    `MotivationManager.update_last_interaction()` は正しく実装されていたが、通常会話処理から**一度も呼び出されていなかった**ため、クールダウンがリセットされなかった。

*   **原因:**
    関数を「実装した」時点で満足してしまい、**その関数がどこから呼び出されるのか**の確認が漏れていた。

*   **解決策:**
    `ui_handlers.py` の `_stream_and_handle_response` の finally ブロックで呼び出しを追加。
...
*   **教訓:**
    関数を新規作成または修正した際は、以下の2点を必ず確認すること：
    1. **関数の実装**は正しいか
    2. **関数の呼び出し**は適切な場所で行われているか
