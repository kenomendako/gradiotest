...
- [x] [ウォッチリストの定時巡回が発火しない問題]
  - ✅ 完了 (2026-01-05)
  - 原因: `get_due_entries()`で「24時間経過 AND 時刻一致」という誤った条件を使用していた。翌朝07:00時点では前日23:00からの経過時間が8時間しかないため24時間条件を満たさず発火しなかった。
  - 修正: 「今日の指定時刻を過ぎており、かつ最後のチェックがそれより前」という条件に変更。
  - 優先度: 完了

- [x] [自律行動の間隔無視問題]
  - ✅ 完了 (2026-01-05)
  - 原因1: ユーザーとの対話（手動チャット）時に退屈度はリセットされていたが、自律行動用のクールダウンタイマーが更新されていなかった。→ `MotivationManager.update_last_interaction` 内で自律行動タイマーもリセットするように変更。
  - 原因2: `update_last_interaction()` 関数が通常会話処理から呼び出されていなかった。→ `ui_handlers.py` の `_stream_and_handle_response` の finally ブロックで呼び出すように修正。
  - 優先度: 完了
