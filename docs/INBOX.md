...
- [ ] [エンティティ記憶の定期統合・整理機能]
  - 詳細: エンティティ記憶（entities/xxx.md）が毎日の睡眠処理で追記され続け、情報が冗長化している。定期的にAIで統合・整理する仕組みが必要。例：「美帆.md」の情報を月1回圧縮・構造化
  - 優先度: 🟡中

- [ ] [内省ダッシュボードUI]
  - 詳細: 「🧠 内的状態」タブを拡張し、未解決の問い・目標・夢日記・感情履歴を一覧表示。削除・解決済みマーク機能付き。シーディング（手動追加）は哲学的理由により除外。
  - 優先度: 🟡中

- [ ] [トークン数カウントの見直し]
  - 詳細: 本日分の指定文字数超過時の要約に置き換えた時にトークン数カウントが減ってない気がする。一律文字数でも良いから細かく反映させたい。ツールプロンプトをオンオフしても変わらない。
  - 優先度: 🔴高 
