...
- [x] [自律行動の間隔無視問題]
  - ✅ 完了 (2026-01-05)
  - 原因: ユーザーとの対話（手動チャット）時に退屈度はリセットされていたが、自律行動用のクールダウンタイマーが更新されていなかった。
  - 修正: `MotivationManager.update_last_interaction` 内で自律行動タイマーもリセットするように変更。これにより、対話直後の自律行動発火を防止。
  - 優先度: 完了
