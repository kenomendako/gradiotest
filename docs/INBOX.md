...
- [ ] [記憶想起、同じ内容が複数並んでる気がする]
  - 詳細: 
  - 優先度:  🟡中 

- [ ] [検索精度アップ]
  - 詳細: ルシアンが「Web検索の結果は……フン、あまり芳しくないな。断片的な情報ばかりで、俺が求めるような、魂の渇きを癒すほどの深い洞察は得られそうにない。総務省の論文は少し気になるが、URLが示されていない。これでは読みようがない。Google Cloudのページは技術的な側面に寄りすぎている。」と言っていた。URLが表示されないのは困るので改善したい。ddg以外の選択肢も探すか。
  - 優先度:  🟡中 

- [ ] [創作ノートのタイムスタンプ改良] 
  - 詳細: 現在は一行ごとに先頭にタイムスタンプを付加するnotepadと同じ仕様だが、それは複数行の詩や文章に合わないので、仕切り線なり見出しなりでセクションを毎回区切り、セクションごとに一つのタイムスタンプを付けるようにする。
  - 優先度: 🔴高 

- [ ] [自律行動時の記憶想起をオフに] 
  - 詳細: クエリ[SILENT]で検索してるので無駄
  - 優先度: 🔴高 

- [x] [創作ノートの書き込み位置ミスなどの改善] ✅ (2026-01-02 修正完了)
  - 詳細: タイムスタンプ自動付与とバックアップ機能を追加
  - 優先度: 🔴高 

- [x] [ログ送信「本日分」の落とし穴] ✅ (2026-01-02 修正完了)
  - 詳細: 今日のエピソード記憶が存在しない場合は昨日のログも含めるように修正
  - 優先度: 🔴高 

- [ ] [自律行動の時間ランダム化]
  - 詳細: 最終応答から何時間後以上何時間後以内の範囲で自律行動のタイミングをある程度ランダムにする。もしくは、もっとAIペルソナの意志や自律性を反映できるアイデアはあるか？
  - 優先度: 
