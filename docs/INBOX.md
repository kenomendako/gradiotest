...
- [ ] [情景画像生成が勝手に行われる問題]
  - 詳細: 以前は無料画像生成APIが使えたので自動生成にしていたが、今は有料なので勝手に生成しないようにしたはずがまた自動生成画復活している。
  - 優先度: 🔴高 
...
- [x] [記憶検索の重複を除去するようにする] ✅ **fix/rag-search-deduplication ブランチで修正完了**
  - 詳細: RAG検索で同一コンテンツが複数返される問題を修正
  - 修正: `RAGManager.search`メソッドにコンテンツベースの重複除去（先頭100文字で判定）を追加
...
- [x] [エンティティ記憶検索が常にヒットしない問題] ✅ **fix/entity-search-query-split ブランチで修正完了**
  - 詳細: retrieval_nodeでエンティティ記憶検索が毎回「なし」になっている。EntityMemoryManager.search_entriesのロジックを調査し、検索がヒットしない原因を特定する。
  - 原因: `search_entries`がクエリ全体を部分一致検索していたため、複数単語のキーワード群でヒットしなかった
  - 修正: クエリを単語分割し、各単語でマッチング。マッチ数でスコアリングして関連度順ソート
