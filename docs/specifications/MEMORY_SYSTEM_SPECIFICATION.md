...
**セッション単位エピソード記憶 (2026-01-15)**:
- 日単位からセッション（会話）単位へ変更
- `session_arousal.json` にタイムスタンプ付きで記録
- Arousal連動の詳細度:
    - `≥0.6` (高): 300文字程度で詳細に
    - `0.3-0.6` (中): 150文字程度で簡潔に
    - `<0.3` (低): 50文字以内で1行に
- 睡眠処理で `update_memory_by_session()` を使用
...
#### Phase 1.5: 複合スコアリング [NEW 2026-01-14]

検索結果を類似度だけでなく、**Arousal（感情的重要度）**を加味してリランキング:
...
|-----------|-----|------|
| **α (ALPHA)** | 0.7 | 類似度の重み |
| **β (BETA)** | 0.3 | Arousalの重み |

**動作**:
- Arousalが高い記憶（感情的に重要）は複合スコアが低くなり、上位にランク
- メタデータに `arousal` がない場合はデフォルト値 0.5（中立）
- 高Arousal（>0.6）の記憶はログに★マーク付きで表示

**ログ出力例**:
```
✅ Sim: 0.408 | Arousal: 0.50 | Comp: 0.435 | [記憶内容]...
```

> [!NOTE]
> **時間減衰（Recency）は意図的に廃止**
> Nexus Arkは対話・共生のためのアプリであり、「古い記憶も大切な思い出」という哲学を尊重。
> 新しい記憶を優先する時間減衰は採用しない。
