...
def generate_alarm_message(character_name: str, context_memo: str, api_key_name: str) -> str:
    """
    アラームのコンテキストメモに基づき、専用のプロンプトを使用して応答メッセージを生成する。
    """
    ...
    try:
        ...
        if hasattr(response, 'text') and response.text:
            ...
            # 万が一 text 属性がない、または空の場合のフォールバック
            # generate_content の仕様上、通常は起こらないはずだが念のため
            print("  - 警告: APIからテキスト応答がありませんでした。")
            # 候補(candidates)からテキストを抽出する試み
            try:
                candidate_text = response.candidates[0].content.parts[0].text
                if candidate_text:
                    generated_text = candidate_text.strip()
                    print(f"  - 候補からテキストを抽出しました: {generated_text}")
                    return generated_text
            except (IndexError, AttributeError):
                pass # 抽出失敗
            return f"（時間になりました。{context_memo}）" # 最終フォールバック

    except Exception as e:
        print(f"--- アラームメッセージ生成中にエラー ---")
        traceback.print_exc()
        return f"（時間になりました。{context_memo}）" # エラー時はメモをそのまま返す
