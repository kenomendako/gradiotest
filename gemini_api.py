...
import filetype # ★ ファイル冒頭に追加

def invoke_nexus_agent(*args: Any) -> str:
    """エージェントを起動し、最終的な応答を取得するメイン関数（マルチメディア対応版）"""
    ...
    if file_input_list:
        genai.configure(api_key=api_key) # Google SDKにAPIキーを設定
        for file_obj in file_input_list:
            ...
            try:
                ...
                if kind is None:
                    # filetypeで判定できない場合は、テキストとして読み込みを試す
    ...
    if user_message_parts:
        # LangChainは多様な型のリストをcontentとして受け取れる
