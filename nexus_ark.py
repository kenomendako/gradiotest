                ...
                with gr.Row(equal_height=False):
                    ...
                    with gr.Column(scale=2, min_width=300):
                            ...
                            with gr.Tabs() as settings_tabs:
                                ...
                                with gr.TabItem("個別") as individual_settings_tab:
                                    ...
                                    with gr.Accordion("📜 ストリーミング表示設定", open=False):
                                        ...
                                        streaming_speed_slider = gr.Slider(
                                            minimum=0.0, maximum=0.1, step=0.005, label="表示速度",
                                            info="値が小さいほど速く、大きいほどゆっくり表示されます。(0.0で最速)", interactive=True
                                        )
                                ...
                                with gr.TabItem("🎨 パレット") as theme_tab:
                                    ...
                                    AVAILABLE_FONTS = sorted([
                                        "Alice", "Archivo", "Bitter", "Cabin", "Cormorant Garamond", "Crimson Pro", "Dm Sans",
                                        "Eczar", "Fira Sans", "Glegoo", "IBM Plex Mono", "Inconsolata", "Inter", "Jost", "Lato",
                                        "Libre Baskerville", "Libre Franklin", "Lora", "Merriweather", "Montserrat", "Mulish",
                                        "Noto Sans", "Noto Sans JP", "Open Sans", "Playfair Display", "Poppins", "Pt Sans",
                                        "Pt Serif", "Quattrocento", "Quicksand", "Raleway", "Roboto", "Roboto Mono", "Rubik",
                                        "Source Sans Pro", "Source Serif Pro", "Space Mono", "Spectral", "Sriracha", "Titillium Web",
                                        "Ubuntu", "Work Sans"
                                    ])
                        ...
                        with gr.Accordion("🧑‍🤝‍🧑 グループ会話", open=False):
                            ...
                            participant_checkbox_group = gr.CheckboxGroup(
                                label="会話に招待するルーム", choices=sorted([c for c in room_list_on_startup if c != effective_initial_room]), interactive=True
                            )
                            with gr.Row():
                            ...
                            with gr.Tabs():
                                with gr.TabItem("📸 スクリーンショット支援"):
                                    ...
                                    screenshot_mode_checkbox = gr.Checkbox(
                                        label="スクリーンショットモードを有効にする", info="有効にすると、下のルールに基づいてチャット履歴の表示が置き換えられます。"
                                    )
                                    with gr.Row():
                                        ...
                                        with gr.Column(scale=3):
                                            ...
                                            redaction_rules_df = gr.Dataframe(
                                                headers=["元の文字列 (Find)", "置換後の文字列 (Replace)"], datatype=["str", "str"],
                                                row_count=(5, "dynamic"), col_count=(2, "fixed"), interactive=False
                                            )
                    ...
                    with gr.Column(scale=6):
                        chatbot_display = gr.Chatbot(
                            height=480, elem_id="chat_output_area", show_copy_button=True, show_label=False,
                            render_markdown=True, group_consecutive_messages=False
                        )
                        with gr.Row():
                        ...
                        chat_input_multimodal = gr.MultimodalTextbox(
                            ...
                            max_plain_text_length=100000, placeholder="メッセージを入力し、ファイルをドラッグ＆ドロップまたは添付してください...",
                            show_label=False, lines=3, interactive=True
                        )
                        token_count_display = gr.Markdown(
                            "入力トークン数: 0 / 0（Gemini2.5無料枠TPM:250,000）", elem_id="token_count_display"
                        )
                        with gr.Row():
                    ...
                    with gr.Column(scale=3, min_width=300):
                        with gr.Accordion("👤 プロフィール・情景", open=True):
                            ...
                            location_dropdown = gr.Dropdown(label="現在地 / 移動先を選択", interactive=True)
